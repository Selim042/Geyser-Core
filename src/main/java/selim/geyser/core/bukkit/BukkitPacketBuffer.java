...
public class BukkitPacketBuffer extends ByteBuf {

	private final ByteBuf buf;

	public BukkitPacketBuffer(ByteBuf wrapped) {
		this.buf = wrapped;
	}
	...
	public BukkitPacketBuffer writeByteArray(byte[] array) {
	...
	/**
	 * Writes an array of VarInts to the buffer, prefixed by the length of the
	 * array (as a VarInt).
	 */
	public BukkitPacketBuffer writeVarIntArray(int[] array) {
		this.writeVarInt(array.length);
	...
	public int[] readVarIntArray(int maxLength) {
		...
		if (i > maxLength) {
			throw new DecoderException(
					"VarIntArray with size " + i + " is bigger than allowed " + maxLength);
		} else {
			int[] aint = new int[i];
	...
	/**
	 * Writes an array of longs to the buffer, prefixed by the length of the
	 * array (as a VarInt).
	 */
	public BukkitPacketBuffer writeLongArray(long[] array) {
		this.writeVarInt(array.length);
	...
	public long[] readLongArray(@Nullable long[] array, int maxLength) {
		...
		if (array == null || array.length != i) {
			if (i > maxLength) {
				throw new DecoderException(
						"LongArray with size " + i + " is bigger than allowed " + maxLength);
			}
	...
	public BukkitPacketBuffer writeUniqueId(UUID uuid) {
	 ...
	 */
	public BukkitPacketBuffer writeVarInt(int input) {
		while ((input & -128) != 0) {
	...
	public BukkitPacketBuffer writeVarLong(long value) {
		while ((value & -128L) != 0L) {
	...
	/**
	 * Reads a compressed NbtCompound from this buffer
	 */
	@Nullable
	public NbtCompound readCompoundTag() throws IOException {
		StreamSerializer serializer = StreamSerializer.getDefault();
		...
		if (b0 == 0) {
			...
			try {
				return serializer
						.deserializeCompound(new DataInputStream((new ByteBufInputStream(this))));
				// return CompressedStreamTools.read(new
	...
	public BukkitPacketBuffer writeItemStack(ItemStack stack) {
		// StreamSerializer serializer = StreamSerializer.getDefault();
		// try {
		// serializer.serializeItemStack(new DataOutputStream(new
		// ByteBufOutputStream(this)), stack);
		// } catch (IOException e) {
		...
		if (stack == null || stack.getType() == Material.AIR) {
			...
			this.writeShort(stack.getTypeId());
			...
			// NbtCompound = stack.getItem().getNBTShareTag(stack);
			// }
	...
	public ItemStack readItemStack() throws IOException {
		...
		if (i < 0) {
			...
			ItemStack itemstack = MinecraftReflection
					.getBukkitItemStack(new ItemStack(Material.getMaterial(i), j, k));
			// itemstack.setTagCompound(this.readCompoundTag());
			NbtFactory.setItemTag(itemstack, this.readCompoundTag());
			return itemstack;
		}
	}
	...
	public BukkitPacketBuffer writeString(String string) {
	...
	// }
	//
	// public BukkitPacketBuffer writeResourceLocation(ResourceLocation
	// resourceLocationIn) {
	...
	public BukkitPacketBuffer writeTime(Date time) {
	...
	public ByteBuf getBytes(int p_getBytes_1_, byte[] p_getBytes_2_, int p_getBytes_3_,
			int p_getBytes_4_) {
		return this.buf.getBytes(p_getBytes_1_, p_getBytes_2_, p_getBytes_3_, p_getBytes_4_);
	}
	...
	public int getBytes(int p_getBytes_1_, FileChannel p_getBytes_2_, long p_getBytes_3_,
	...
	public CharSequence getCharSequence(int p_getCharSequence_1_, int p_getCharSequence_2_,
		...
		return this.buf.getCharSequence(p_getCharSequence_1_, p_getCharSequence_2_,
				p_getCharSequence_3_);
	}
	...
	public ByteBuf setBytes(int p_setBytes_1_, byte[] p_setBytes_2_, int p_setBytes_3_,
			int p_setBytes_4_) {
		return this.buf.setBytes(p_setBytes_1_, p_setBytes_2_, p_setBytes_3_, p_setBytes_4_);
	}
	...
	public int setBytes(int p_setBytes_1_, FileChannel p_setBytes_2_, long p_setBytes_3_,
	...
	public int setCharSequence(int p_setCharSequence_1_, CharSequence p_setCharSequence_2_,
		...
		return this.buf.setCharSequence(p_setCharSequence_1_, p_setCharSequence_2_,
				p_setCharSequence_3_);
	}
	...
	public int readBytes(FileChannel p_readBytes_1_, long p_readBytes_2_, int p_readBytes_4_)
			throws IOException {
	...
	public int writeBytes(FileChannel p_writeBytes_1_, long p_writeBytes_2_, int p_writeBytes_4_)
			throws IOException {
	...
	public int forEachByteDesc(int p_forEachByteDesc_1_, int p_forEachByteDesc_2_,
		...
		return this.buf.forEachByteDesc(p_forEachByteDesc_1_, p_forEachByteDesc_2_,
				p_forEachByteDesc_3_);
	}
