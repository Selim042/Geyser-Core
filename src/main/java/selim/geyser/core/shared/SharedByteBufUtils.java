...
public class SharedByteBufUtils {
	...
	/**
	 * The number of bytes to write the supplied int using the 7 bit varint
	 * encoding.
	 *
	 * @param toCount
	...
	/**
	 * Write an integer to the buffer using variable length encoding. The
	 * maxSize constrains how many bytes (and therefore the maximum number) that
	 * will be written.
	 *
	 * @param to
	...
	/**
	 * Read a UTF8 string from the byte buffer. It is encoded as <varint
	 * length>[<UTF8 char bytes>]
	 *
	 * @param from
	 *            The buffer to read from
	 * @return The string
	...
	public static void writeUTF8String(ByteBuf to, String string) {
		...
		Validate.isTrue(varIntByteCount(utf8Bytes.length) < 3,
